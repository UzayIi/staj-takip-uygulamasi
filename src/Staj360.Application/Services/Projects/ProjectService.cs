...
public class ProjectService : IProjectService
{
    ...
    public async Task<PagedResult<Project>> ListAsync(int page, int pageSize, CancellationToken cancellationToken = default)
    {
        page = page < 1 ? 1 : page;
        pageSize = pageSize is < 1 or > 200 ? 20 : pageSize;

        var query = _db.Projects.AsNoTracking().Include(p => p.Department).Where(p => !p.IsDeleted);
    ...
    public async Task<IReadOnlyList<Project>> ListForMentorAsync(Guid mentorUserId, CancellationToken cancellationToken = default) =>
        await _db.Projects.AsNoTracking().Include(p => p.Department)
    ...
    public async Task<Project?> GetAsync(Guid id, CancellationToken cancellationToken = default) =>
        await _db.Projects.Include(p => p.Department)
    ...
    public async Task<IReadOnlyList<Project>> ListForInternAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        return await _db.Projects.AsNoTracking().Include(p => p.Department)
