...
public class InternService : IInternService
{
    ...
    public async Task<PagedResult<InternListItemDto>> ListAsync(string? search, int? year, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        page = page < 1 ? 1 : page;
        ...
        var query =
            ...
            join d in _db.Departments.AsNoTracking() on i.DepartmentId equals d.Id
            where !i.IsDeleted
    ...
    public async Task<InternProfile?> GetAsync(Guid id, CancellationToken cancellationToken = default) =>
        await _db.InternProfiles.Include(i => i.Department).FirstOrDefaultAsync(i => i.Id == id && !i.IsDeleted, cancellationToken);

    public async Task<InternProfile?> GetByUserIdAsync(Guid userId, CancellationToken cancellationToken = default) =>
        await _db.InternProfiles.Include(i => i.Department).FirstOrDefaultAsync(i => i.UserId == userId && !i.IsDeleted, cancellationToken);
