...
public class InternsController : Controller
{
    ...
    public InternsController(IInternService internService, IDepartmentService departmentService, IUserAccountService accountService, IApplicationDbContext db)
    {
    ...
    public async Task<IActionResult> Index(string? search, int? year, int page = 1, CancellationToken cancellationToken = default)
    {
        var result = await _internService.ListAsync(search, year, page, 20, cancellationToken);
        ViewData["Search"] = search;
        ViewData["Year"] = year;

        // Mevcut staj dönemlerinden yıl listesi çıkar.
    ...
    public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken)
    {
        ...
        TempData[result.Success ? "Success" : "Error"] = result.Success ? "Stajyer başarıyla silindi." : result.ErrorMessage;
        return RedirectToAction(nameof(Index));
    }
}
